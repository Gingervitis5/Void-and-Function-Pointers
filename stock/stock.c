...
void * buildStock(FILE * fin) {
    Stock *stock = (Stock *) calloc(1, sizeof(Stock)); char temp[100]; double price;
    stock->symbol = (char *)calloc(100, sizeof(char));
    stock->name = (char *)calloc(100, sizeof(char));
    fgets(temp, 100, fin);
    strip(temp);
    strcpy(stock->symbol, temp);
    fgets(temp, 100, fin);
    strip(temp);
    strcpy(stock->name, temp);
    fscanf(fin, "%lf", &price);
    stock->price = price;
    fgetc(fin);
    return stock;
...
void cleanStock(void * ptr){
    ...
    free(stock->symbol);
    free(stock->name);
    stock->name = NULL, stock->symbol = NULL;
    free(stock);
    stock = NULL;
}
